...
namespace Default {
    ...
    void App::run()
    {
        RenderWindow window(VideoMode(1850/2,960), m_Title, sf::Style::Titlebar | sf::Style::Close);
        ...
        while(window.isOpen())
        {
            ...
            model.update(window, mousePos.x, mousePos.y);
            window.clear();
            model.draw();
            window.display();
